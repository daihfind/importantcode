...
from mocle.index_compute import mocle_index,Euclidean_dist
from dsce import *
import array
import tables
...
def rsnn(sampledData,remainedData,sampledIndex,remainedIndex,singleName):
    ...
    combineIndex = []
    combinedLables = []
    for row in range(len(predicted_labelAll)):
        combineIndexOne = sampledIndex[row] + remainedIndex[row]
        combinedLablesOne = predicted_labelAll[row] + assinALLNnLabels[row]
...
def multirun(datasetName):
    ...
    for i in range(generation):
        ...
        for subpop in subpopArr:
            ...
            resultList = CE.CSPA(hdf5_file_name, subpop, verbose=True, N_clusters_max=3)
