...
#dsce的第二阶段工作
#返回:新的相似矩阵θ1，以及对应合并后的候选矩阵,以及合并的原始簇的序号集合的集合
def measureSimilarity(transMatrix,popClusterArr_3,popClusterArr_2,clusterNumArr,datamat,a1):
    ...
    unionClusterArr_2_index = [] #相似矩阵合并二维数组中对应的初始簇序号
    ...
    for key in dictSimi:
        ...
        # 对应序号数组
        unionArrIndex = []
        unionArrIndex.append(key)
        unionArrIndex.extend(clusterSimiIndexArr)
        unionClusterArr_2_index.append(unionArrIndex)
        #合并相似的矩阵，去除重复的元素
        ...
        j += 1
    # similarMatrix1 = moreUnion(unionClusterArr_2, a1, datamat, similarMatrix,unionClusterArr_2_index,transMatrix)
    return similarMatrix


# 初次合并后的簇再计算合并后的簇的相似度，大于a１的继续进行合并
def moreUnion(unionClusterArr_2, a1,datamat,oldSimiliarMatrix,unionClusterArr_2_index,transMatrix):
    ...
    if flag == 1:
        ...
        #新的合并的数组的特殊处理,处理重复值
        transUnionIndexArr = []
        for ui in sortedUnionIndexArr:
            trans = []
            for subui in ui:
                trans.extend(list(set(trans).union(set(unionClusterArr_2_index[subui]))))
            transUnionIndexArr.append(trans)
        for i in range(len(datamat)):
            j = 0
            for unionindex in transUnionIndexArr:
                for index1 in unionindex:
                    similiarMatrix[i][j] += transMatrix[i][index1]
                j += 1
            for ununionindex in sortedUnunionIndexArr:
                similiarMatrix[i][j] += oldSimiliarMatrix[i][ununionindex]
                j += 1
        newunionClusterArr_2_index = transUnionIndexArr
        for unui in sortedUnunionIndexArr:
            newunionClusterArr_2_index.append(unionClusterArr_2_index[unui])
        return moreUnion(newunionClusterArr_2,a1,datamat,similiarMatrix,newunionClusterArr_2_index,transMatrix)
#dsce的第三阶段工作
def assign(similiarMatrix,a2):
    ...
    dictCQ = getCQ(dictCownP,similiarMatrix)
    ...
    for unIndex in uncertainP:
        minDiffer = inf
        assignClu = -1
        for canCluIndex in candidateClusterArr:
            tmpDiffer = abs((float(similiarMatrix[unIndex][canCluIndex])-dictCQ[canCluIndex])/(len(dictCownP[canCluIndex])+1))
            #todo 上下两个等式等同，上面的那个化简过
            # tmpDiffer = abs((float(similiarMatrix[unIndex][canCluIndex])+dictCQ[canCluIndex]*len(dictCQ[canCluIndex]))/(len(dictCownP[canCluIndex])+1)-dictCQ[canCluIndex])
            if tmpDiffer < minDiffer:
                minDiffer = tmpDiffer
                assignClu = canCluIndex
        if assignClu == -1:
            print similiarMatrix
            print len(candidateClusterArr)
            print '-1'
        dictCownP[assignClu].append(unIndex)
    # if len(dictCownP) == 1:
    #     print '重新分配一次,再来再来!'
    #     a2 = a2 - 0.1
    #     if a2 < 0.1:
    #         print '不行了不行了不行列不行了不行了不行了不行列不行了不行了不行了不行列不行了'
    #     return assign(similiarMatrix, a2)
    return dictCownP
...
def nomalise(similiarityMatrix):
    ...
    for i in range(len(similiarityMatrix)):
        for j in range(len(similiarityMatrix[0])):
            similiarityMatrix[i][j] = float(similiarityMatrix[i][j]-minValue)/interval
