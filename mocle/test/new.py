...
def main():
    ...
    sampledData, remainedData, sampledIndex, remainedIndex ,sampledDataFs= data_samplefs(datamat,1,30)
    print 'sampledData ready'

    # pop_kmeans = multiRunSelect(sampledData, remainedData, sampledIndex, remainedIndex,sampledDataFs,'kmeans',datamat)
    # print 'kmeans end'
    # pop_ward = multiRunSelect(sampledData, remainedData, sampledIndex, remainedIndex,sampledDataFs,'ward',datamat)
    # print 'ward end'
    # pop_complete = multiRunSelect(sampledData, remainedData, sampledIndex, remainedIndex,sampledDataFs,'complete',datamat)
    # print 'complete end'
    # pop_average = multiRunSelect(sampledData, remainedData, sampledIndex, remainedIndex,sampledDataFs,'average',datamat)
    # print 'average end'
    ...
    for i in range(generation):
        ...
        fitnesses = toolbox.map(toolbox.evaluate, tile(datamat,(len(invalid_ind),1,1)),tile(offspring,(len(invalid_ind),1,1)),invalid_ind)#这里只用了未经处理的数据,没有用到真实类别
        for ind, fit in zip(invalid_ind, fitnesses):
            ind.fitness.values = fit

        # Chossing a population for the next generation
        population = toolbox.select(popElite + offspring, len(population))
